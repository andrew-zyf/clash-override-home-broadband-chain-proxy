...
**调度分配**
- 其他调度（视频 / 音乐 / 社交 / IM）候选增加 `🎯 统一出口`，可一键跟严管同出口；默认仍美区优先。
- 统一出口扁平挂载家宽实体节点（静态 IP / 官方中转），避免只嵌套「家宽出口」组时节点不易见。
...
**家庭静态 IP（SOCKS5）**
- 新增 `RESIDENTIAL_CREDENTIALS.homeStatic`：注入「家宽出口（家庭静态 IP）」socks5 节点，算家宽。
- `transit` / `homeStatic` 统一为 `{ server, port, username, password }`（不再使用顶层用户名密码）。
- 可与官方中转并存；并存时静态 IP 优先。仅静态 IP / 仅中转 / 都没有（降级）均支持。
- `homeStatic` 认证可选；`localhost` 允许，文档占位域名仍拒绝。
...
**空凭证降级**
- `merged` 下 `RESIDENTIAL_CREDENTIALS` 为空或文档占位符时不再抛错中断整段覆写。
- 仍写入 DNS / Sniffer / 严管规则与面板；跳过官方中转注入，清理残留中转节点。
- 家宽出口组降级挂地区测速组（无地区节点时为 `DIRECT`）。
